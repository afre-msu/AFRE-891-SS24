...
### Resources:
- [.hi-dkorange[Vignette]](https://cran.r-project.org/web/packages/tidyr/vignettes/tidy-data.html) (from the **tidyr** package)
- [.hi-dkorange[Original paper]](https://vita.had.co.nz/papers/tidy-data.pdf) (Hadley Wickham, 2014 JSS)
- [.hi-dkorange[Online Book: R 4 Data Science]](https://r4ds.hadley.nz/)
...
Basically, tidy data is more likely to be [.hi-dkorange[long (i.e. narrow) format]](https://en.wikipedia.org/wiki/Wide_and_narrow_data) than wide format.
...
1. [.hi-dkorange[dplyr]](https://dplyr.tidyverse.org/)
2. [.hi-dkorange[tidyr]](https://tidyr.tidyverse.org/)
...
# Pipes
 ...
 Suppose you're a big German car fan and want to see average fuel efficiency of their models for 1999-2008. The simple operations involve 
...
PS — The pipe is originally from the [.hi-dkorange[magrittr]](https://magrittr.tidyverse.org/) package (hence the [.hi-dkorange[not-a-pipe image]](https://en.wikipedia.org/wiki/The_Treachery_of_Images)) earlier, which can do some other cool things if you're inclined to explore.
...
The magrittr pipe has proven so successful and popular, that as of R 4.1.0 the R core team [.hi-dkorange[added a "native" pipe]](https://www.tidyverse.org/blog/2023/04/base-vs-magrittr-pipe/), denoted `|>`.<sup>1</sup> For example:
...
- Please make sure that you are running at least .hi-slate[dplyr] 1.0.0 before continuing.
...
`arrange()` sorts rows based on values of a variable/variables
...
We can also arrange items in .hi-purple[descending order] using `arrange(desc())`.
...
`summarise()` lets us .medgrn[manually specify summary statistics]. It's particularly useful in combination with the `group_by` command.
...
There are also a whole class of [.hi-dkorange[window functions]](https://cran.r-project.org/web/packages/dplyr/vignettes/window-functions.html) for getting leads and lags, ranking, creating cumulative aggregates, etc.
...
One of the mainstays of the dplyr package is merging data with the family [.hi-dkorange[join operations]](https://cran.r-project.org/web/packages/dplyr/vignettes/two-table.html).
- `inner_join(df1, df2)`
- `left_join(df1, df2)`
- `right_join(df1, df2)`
- `full_join(df1, df2)`
...
(You can find some helpful visual depictions of the different join operations [.hi-dkorange[here]](https://r4ds.had.co.nz/relational-data.html).)
...
For our join examples, we'll use some data sets that come bundled with the [.hi-dkorange[nycflights13]](http://github.com/hadley/nycflights13) package.
...
.footnote[<sup>4</sup> These datasets are technically stored as tibbles, which are an [.hi-dkorange[opinionated, modern version of data frames]](https://tibble.tidyverse.org/). For our uses we can treat them essentially interchangeably, or forcibly go between types with `as.data.frame()/as.tibble()`
]
...
Let's perform a [.hi-dkorange[left join]](https://stat545.com/bit001_dplyr-cheatsheet.html#left_joinsuperheroes-publishers) to bring variables from the planes dataset into the flights dataset.
