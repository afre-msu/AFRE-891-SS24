...
First for the non-clustered forest:
...
.hi-medgrn[Solution:] use a new "simulated" covariate matrix to predict treatment effects if schools had different achievement levels, holding other characteristics fixed.
...
sim_x2 <- function(val){
  ...
  X <- mutate(Xmat, X2 = val)
  
  # Estimate forest
  cf_sim <- causal_forest(X = X_sel, 
                    Y = Y, 
                    W = Z, 
                    num.trees = 200, # fewer trees for expediency of estimation
                    honesty = TRUE,
                    honesty.fraction = 0.5,
                    tune.parameters = "all" 
                    )
  
  res <- data.frame(preds = predict(cf_sim)$predictions) %>%
    mutate(X2_val = val)
  return(res)
}
```
...
sim_x2 <- map_dfr(x2_seq, sim_x2)
saveRDS(sim_x2, "output/x2_sim_df.rds")
```
...
sim_x2 <- readRDS("output/x2_sim_df.rds")
...
# Plotting
ggplot(data = sim_x2, aes(x = factor(X2_val), y =preds)) +
  geom_boxplot() +
  theme_minimal() +
...
cf_sel_ll <- causal_forest(
  ...
  X = X_sel, Y = Y, W = Z, clusters = nslm$schoolid, equalize.cluster.weights = TRUE, num.trees = 2000, honesty = TRUE, honesty.fraction = 0.5,  tune.parameters = "all",
  # changed inputs:
                    Y.hat = Y_ll_hat, #<<
                    W.hat = W_ll_hat, #<<
                    )
